...
construct a new query see (limitation)[#Max results limitation].
...
key <- read.delim("key.secret") # NOTE: Loads the API key from a local file, 
...
for (i in 1:total_runs) {
    ...
    '
    query {
        search(query: ', stars_query, ', type: REPOSITORY, first: ', results_per_page, ',', page_key_query, '){
            repositoryCount
                edges {
                    node {
                        ... on Repository {
    ...
    if (!data$search$pageInfo$hasNextPage) {
        ...
        stars_query <- paste('"stars:100..', data$search$edges[[results_per_page]]$node$stargazerCount - 1, '"', sep = '')
        print(stars_query)
        page_key_query <- ""
    } else {
...
for (edge in repo_edges) {
    repo <- edge$node
    bug_labels <- Filter(function(label) grepl("bug", label$name, ignore.case = TRUE),repo$labels$nodes)
    bug_issues <- 0
    bug_pull_requests <- 0
...
```{r}
repos_fmt <- repos[repos$bug_issues != 0 | repos$bug_pull_requests != 0, ]
# TODO: Filter on language == NULL
```
